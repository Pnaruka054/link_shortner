...
const AllLinks = () => {
    ...
    let Hide_url_btn = async (shortID) => {
        if (confirm("Are you sure?")) {
            setHide_process_state(false)
            let fatchData = async () => {
                try {
                    let response = await axios.put(`${import.meta.env.VITE_SERVER_URL}/linkShort/hide_and_show_link?shortID=${shortID}&status=show`)
                    if (response.data.success) {
                        setSuccess(`The Link with alias: ${shortID} has been hidden.`)
                        setTimeout(() => {
                            setSuccess('')
                        }, 5000);
                        setHide_process_state(true)
                    }
                } catch (error) {
                    console.log(error);
                }
                                ...
                                <div className="row">
                                    ...
                                    <div className="col-sm-3 col-lg-2 mt-2 m-md-auto">
                                        <div className="text-right d-flex flex-nowrap">
                                            ...
                                            <a href="#" disabled={!hide_process_state} className={`btn ${!hide_process_state ? "btn-secondary" : "btn-danger"} mx-2 btn-sm`} onClick={() => Hide_url_btn(link.shortURL_ID)}>
                                                {hide_process_state ? "Hide" : <i className="fa-sharp-duotone fa-solid fa-loader fa-spin-pulse"></i>}
                                            </a>
                                        </div>
                                    </div>
                                </div>
